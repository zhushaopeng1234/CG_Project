...
Cursor _glfwCreateCursorX11(const GLFWimage* image, int xhot, int yhot)
{
    ...
    native->xhot = (unsigned int)xhot;
    native->yhot = (unsigned int)yhot;
	...
	*target = (alpha << 24) |
	          (unsigned int)((unsigned char) ((source[0] * alpha) / 255) << 16) |
	          (unsigned int)((unsigned char) ((source[1] * alpha) / 255) <<  8) |
	          (unsigned int)((unsigned char) ((source[2] * alpha) / 255) <<  0);
    }
