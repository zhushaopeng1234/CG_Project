...
void _glfwPollMonitorsX11(void)
{
    if (_glfw.x11.randr.available && !_glfw.x11.randr.monitorBroken)
    {
        ...
        {
		disconnected = calloc((unsigned long)_glfw.monitorCount, sizeof(_GLFWmonitor*));
            memcpy(disconnected,
                   _glfw.monitors,
	           (unsigned long)_glfw.monitorCount * sizeof(_GLFWmonitor*));
        }
...
void _glfwPlatformGetMonitorWorkarea(_GLFWmonitor* monitor, int* xpos, int* ypos, int* width, int* height)
{
    ...
    if (_glfw.x11.NET_WORKAREA && _glfw.x11.NET_CURRENT_DESKTOP)
    {
                                      ...
                                      (unsigned char**) &desktop) > 0)
        {
            ...
            {
		const int globalX = (int)extents[*desktop * 4 + 0];
		const int globalY = (int)extents[*desktop * 4 + 1];
		const int globalWidth  = (int)extents[*desktop * 4 + 2];
		const int globalHeight = (int)extents[*desktop * 4 + 3];
...
GLFWvidmode* _glfwPlatformGetVideoModes(_GLFWmonitor* monitor, int* count)
{
    ...
    *count = 0;
	...
	result = calloc((unsigned int)oi->nmode, sizeof(GLFWvidmode));
...
GLFWbool _glfwPlatformGetGammaRamp(_GLFWmonitor* monitor, GLFWgammaramp* ramp)
{
    ...
    {
	const unsigned int size = (unsigned int) XRRGetCrtcGammaSize(_glfw.x11.display,
                                                monitor->x11.crtc);
	...
	_glfwAllocGammaArrays(ramp,(unsigned int) size);

        XF86VidModeGetGammaRamp(_glfw.x11.display,
                                _glfw.x11.screen,
	                        (int)ramp->size, ramp->red, ramp->green, ramp->blue);
        return GLFW_TRUE;
    }
    else
    {
...
void _glfwPlatformSetGammaRamp(_GLFWmonitor* monitor, const GLFWgammaramp* ramp)
{
	...
	XRRCrtcGamma* gamma = XRRAllocGamma((int)ramp->size);
