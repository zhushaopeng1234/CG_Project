...
{
    (void)display;
    _GLFWwindow* window = (_GLFWwindow*) (void*)pointer;
...
{
    (void)display;
    XEvent* notification = (XEvent*) (void*)pointer;
    return event->type == PropertyNotify &&
           event->xproperty.state == PropertyNewValue &&
    ...
    if (window->monitor)
    {
        ...
        if (_glfw.x11.NET_WM_STATE && _glfw.x11.NET_WM_STATE_FULLSCREEN)
        {
            sendEventToWM(window,
                          ...
                          _NET_WM_STATE_ADD,
	                  (long)_glfw.x11.NET_WM_STATE_FULLSCREEN,
                          0, 1, 0);
        }
        else
        {
...
static char** parseUriList(char* text, int* count)
{
    ...
    *count = 0;
	...
	paths = realloc(paths, (unsigned int)(*count) * sizeof(char*));
        paths[*count - 1] = path;
...
static size_t encodeUTF8(char* s, unsigned int ch)
{
    ...
    {
	s[count++] = (char)(ch >> 6) | 0xc0;
	s[count++] = (char)(ch & 0x3f) | 0x80;
    }
    else if (ch < 0x10000)
    {
	s[count++] = (char)(ch >> 12) | 0xe0;
	s[count++] = (char)((ch >> 6) & 0x3f) | 0x80;
	s[count++] = (char)(ch & 0x3f) | 0x80;
    }
    else if (ch < 0x110000)
    {
	s[count++] = (char)(ch >> 18) | 0xf0;
	s[count++] = (char)((ch >> 12) & 0x3f) | 0x80;
	s[count++] = (char)((ch >> 6) & 0x3f) | 0x80;
	s[count++] = (char)(ch & 0x3f) | 0x80;
    }
...
static char* convertLatin1toUTF8(const char* source)
{
    ...
    for (sp = source;  *sp;  sp++)
	tp += encodeUTF8(tp, (const unsigned int)*sp);
...
static void acquireMonitor(_GLFWwindow* window)
{
    ...
    if (window->x11.overrideRedirect)
    {
        ...
        XMoveResizeWindow(_glfw.x11.display, window->x11.handle,
	                  xpos, ypos,(unsigned int) mode.width,(unsigned int) mode.height);
    }
...
static void processEvent(XEvent *event)
{
    ...
    switch (event->type)
    {
	    ...
	    const int mods = translateState((int) event->xkey.state);
            ...
            if (window->x11.ic)
            {
                ...
                if (!filtered)
                {
                    ...
                    {
			chars = calloc((unsigned long)(count + 1), 1);
                        count = Xutf8LookupString(window->x11.ic,
                                                  &event->xkey,
                                                  chars, count,
                                                  NULL, &status);
		...
		const long character = _glfwKeySym2Unicode((unsigned int)keysym);
                if (character != -1)
			_glfwInputChar(window, (unsigned int)character, mods, plain);
            }
        ...
        case ButtonPress:
        {
	    const int mods = translateState((int) event->xbutton.state);
            ...
            else
            {
                ...
                _glfwInputMouseClick(window,
		                     (int)event->xbutton.button - Button1 - 4,
                                     GLFW_PRESS,
                                     mods);
            }
		...
		_glfw.x11.xdnd.source  = (Window) event->xclient.data.l[0];
		_glfw.x11.xdnd.version = (int) event->xclient.data.l[1] >> 24;
		    ...
		    reply.xclient.data.l[0] = (long) window->x11.handle;
